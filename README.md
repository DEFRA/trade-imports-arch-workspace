...
| `tools/` *(workspace convention)* | Per-domain shell scripts that do the deterministic work behind skills - Confluence page operations and publish pre-flight, Jira ticket creation and JQL fetch, Mermaid rendering, skill scaffolding, the workspace and auth doctors. Runnable by hand or from any agent. |
