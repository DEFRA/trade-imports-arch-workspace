...
Canonical state is JSON at `~/trade-imports-arch-workspace/.claude/workareas/test-plain/<id>/state.json`. Schema: `assets/test-plain-schema.md`. Mutated only via `.claude/tools/test-plain/*.sh` helpers (atomic `jq ... > tmp; mv tmp file`).
